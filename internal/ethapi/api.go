...
func (s *BundleAPI) CallBundle(ctx context.Context, args CallBundleArgs, overrides *StateOverride) (map[string]interface{}, error) {
	...
	for i, tx := range args.Transactions {
		...
		bal := state.GetBalance(coinbase)
		balInt := new(big.Int)
		balInt.SetBytes(bal.Bytes())
		beforeBal := new(big.Int)
		beforeBal.SetBytes(coinbaseBalanceBeforeTx.Bytes())
		coinbaseDiffTx := new(big.Int).Sub(balInt, beforeBal)
	...
	balInt.SetBytes(bal.Bytes())
	beforeBal := new(big.Int)
	beforeBal.SetBytes(coinbaseBalanceBefore.Bytes())
	coinbaseDiff := new(big.Int).Sub(balInt, beforeBal)
...
func (s *TransactionAPI) GetBlockReceipts(ctx context.Context, blocks_hash common.Hash) (ReceiptsPackage, error) {
	...
	for i := 0; i < len(receipts); i++ {
		...
		receipt := receipts[i]
		_, tx, blockHash, blockNumber, index, err := s.b.GetTransaction(ctx, receipt.TxHash)
